...
async function installBit(versionMeta, options, onProgress) {
  ...
  if (!installer) {
    ...
    if (!url) {
      throw new Error('未获取到比特安装包下载地址。请在运营后台「环境安装包」上传/填写直链并发布，或点击「安装」选择本地安装包。');
    }
  ...
  if (isWin) {
    if (/\.msi$/i.test(installer)) {
      ...
      throw new Error('暂不支持自动解压 zip，请上传 exe/msi 安装包，或解压后点击「安装」选择本地安装包');
    } else {
...
async function isBitProcessRunning() {
  if (process.platform === 'win32') {
    const script = `
$ErrorActionPreference = 'SilentlyContinue'
$p = Get-Process -ErrorAction SilentlyContinue |
  Where-Object { $_.ProcessName -match 'BitBrowser|bitbrowser|BitBrowserPro|比特' } |
  Select-Object -First 1
if ($p) { Write-Output '1' } else { Write-Output '0' }
`;
    try {
      const r = await runPs(script);
  ...
  if (process.platform === 'darwin') {
    try {
      const r = await runCmd('pgrep', ['-fil', 'BitBrowser'], { shell: false });
      return !!(r && String(r.stdout || '').trim());
    } catch (_) {
      return false;
    }
  }
...
async function launchBit(exePath) {
  const exe = String(exePath || '').trim();
  if (!exe) throw new Error('请先点击「指定位置」选择 BitBrowser.exe');
  if (!fs.existsSync(exe)) throw new Error('比特程序不存在：' + exe);
  if (await isBitProcessRunning()) {
    return { ok: true, alreadyRunning: true, path: exe };
  }
  const api = await probeBitApi();
  if (api && api.online) {
    return { ok: true, alreadyRunning: true, path: exe, viaApi: true };
  }
  ...
  child.unref();
  return { ok: true, alreadyRunning: false, path: exe, pid: child.pid || null };
}
