...
function isLikelyBitExe(filePath) {
  ...
  if (/^bitbrowser(pro)?(\s+global)?\.exe$/i.test(base)) return true;
  if (/比特浏览器\.exe$/i.test(base)) return true;
  // 安装目录名含 BitBrowser / 比特，且是 exe
  if (/\.exe$/i.test(base) && /\\bitbrowser[^\\]*\\|\\比特浏览器\\/i.test(lower)) return true;
...
async function installNode(versionMeta, cacheDir, onProgress) {
  ...
  // 若本机是 nvm-windows，优先用 nvm install（避免与 MSI 冲突）
  if (isWin) {
    const nvm = await resolveNvmWindows();
    if (nvm && nvm.exe) {
      onProgress && onProgress({ stage: 'install', percent: 10, message: `检测到 nvm，正在安装 Node ${versionMeta.id}…` });
      try {
        await runCmd(nvm.exe, ['install', String(versionMeta.id)], { shell: false });
      } catch (e) {
        // nvm 有时把提示打到 stderr，再尝试 use
      }
      onProgress && onProgress({ stage: 'install', percent: 70, message: `正在切换到 Node ${versionMeta.id}…` });
      try {
        await runCmd(nvm.exe, ['use', String(versionMeta.id)], { shell: false });
      } catch (e) {
        throw new Error('nvm 安装/切换失败：' + ((e && e.message) || e) + '。也可在终端执行：nvm install ' + versionMeta.id);
      }
      const after = await detectNode();
      if (!after.installed) {
        throw new Error('nvm 已执行，但未检测到 Node。请新开终端执行 nvm use ' + versionMeta.id + ' 后再点重新检测');
      }
  ...
  return { ok: true, file: dest, method: 'msi' };
}
